...
void DynamicSlice()
{
   ...
   TFile *f = TFile::Open("xsec_file_cv.root");
   TH2D* hpxpy = (TH2D*)f->Get("xsec_muangle_mumom_cv");
...
void DynamicExec()
{
   ...
   Float_t upx = gPad->AbsPixeltoY(py);
   ...
   else   c3 = new TCanvas("c3","Projection Canvas",710,10,700,500);
   c3->SetGrid();
   c3->cd();

   //draw slice corresponding to mouse position
   Int_t binx = h->GetXaxis()->FindBin(x);
   TH1D *hp2 = h->ProjectionY("",binx,binx);
   hp2->SetFillColor(38);
   sprintf(title,"Projection of biny=%d",binx);
   hp2->SetName("Projection");
   hp2->SetTitle(title);
   hp2->Fit("gaus","ql");
   hp2->GetFunction("gaus")->SetLineColor(kRed);
   hp2->GetFunction("gaus")->SetLineWidth(6);
   c3->Update();
   padsav->cd();
}
