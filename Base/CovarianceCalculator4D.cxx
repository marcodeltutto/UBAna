...
namespace Base {
  ...
  void CovarianceCalculator4D::CalculateCovarianceMatrixPoly() 
  {
    ...
    std::cout << _name << " Calculating Cov Matrix with " << _bs_poly.GetNUniverses() << " universes. Nominal histogram is excluded." << std::endl;
    ...
    for (int i = 0; i < n_bins; i++) {
      ...
      for (int j = 0; j < n_bins; j++) {
        ...
        for (int s = 0; s < number_of_universes; s++) {
          ...
          // _bs_poly.NextUniverse(uni_name, uni_histo);
    ...
    // if (_verbose) {
      ...
      for (int i = 0; i < n_bins; i++) {
        for (int j = 0; j < n_bins; j++) {
          std::cout << "(" << i << ", " << j << ") => " << _M_p[i][j] << std::endl;
        }
      }
      ...
      for (int i = 0; i < n_bins; i++) {
        for (int j = 0; j < n_bins; j++) {
          std::cout << "(" << i << ", " << j << ") => " << _M_frac_p[i][j] << std::endl;
        }
      }
    // }
    ...
    for (int i = 0; i < n_bins; i++) {
      ...
      for (int j = 0; j < n_bins; j++) {
        ...
        std::cout << "This is " << i << ", " << j << ", cov matrix is " << _M_p[j][j] << std::endl;
